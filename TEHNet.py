...
class TEHNet(nn.Module):
    ...
    def forward(self, x):
        ...
        hand_trans_right = torch.cat([hand_params[:, 96:98], f], 1) * hand_params[:, 98] - self.roots[0]
        hand_trans_left = torch.cat([hand_params[:, 195:197], f], 1) * hand_params[:, 98] - self.roots[1]
